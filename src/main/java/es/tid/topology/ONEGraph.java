...
public class ONEGraph {
	...
	/*ADDED*/
	HashMap<String, Node> nodesByIntfAddress;
	...
	public void addNode(Node node){
		...
		/*ADDED*/
		/*Each time that we add a node, we also add all its interfaces to the hash table*/
		addAllIntfAddressNode(node);
		
	}
	public void removeNode(String nodeID){
		/*ADDED*/
		/*Each time that we delete a node, we also delete all its interfaces to the hash table*/
		removeAllIntfAddressNode(nodes.get(nodeID));
		nodes.remove(nodeID);
	...
	/*ADDED*/
	public void addAllIntfAddressNode(Node node){		
		Iterator <Intf> intfListIterator=node.getIntfList().iterator();
		while(intfListIterator.hasNext()){
			Intf auxIntf = intfListIterator.next();
			Iterator <String> addressIterator=auxIntf.getAddress().iterator();
			while(addressIterator.hasNext()){	
				String auxAddress = addressIterator.next();
				this.addNodeByIntfAddress(auxAddress, node);
			}
		}
	}
	
	public void addNodeByIntfAddress(String intfname, Node node){
		nodesByIntfAddress.put(intfname, node);
	}
	/*ADDED*/
	public Node getNodeByIntfAddress(String intfname){
		if (nodesByIntfAddress.containsKey(intfname))
			return nodesByIntfAddress.get(intfname);
		else			
			return null;
	}
	/*ADDED*/
	public void removeAllIntfAddressNode(Node node){		
		Iterator <Intf> intfListIterator=node.getIntfList().iterator();
		while(intfListIterator.hasNext()){
			Intf auxIntf = intfListIterator.next();
			Iterator <String> addressIterator=auxIntf.getAddress().iterator();
			while(addressIterator.hasNext()){	
				String auxAddress = addressIterator.next();
				nodesByIntfAddress.remove(auxAddress);			
			}
		}
	}
	...
	public ONEGraph(){
		...
		/*ADDED*/
		nodesByIntfAddress = new HashMap<String, Node> ();
	}
			...
			for (int j = 0; j < nodes_domains.getLength(); j++) {
				...
				if (nodes.getLength()>0){
					for (int i = 0; i < nodes.getLength(); i++) {
						...
						//Configuration Mode
						NodeList configurationMode_node = element.getElementsByTagName("configurationMode");
						String configurationMode = null;
						if (configurationMode_node.getLength()>0){
							Element configurationMode_e = (Element)configurationMode_node.item(0);
							configurationMode = getCharacterDataFromElement(configurationMode_e);
						}
						//Configuration Mode
						NodeList routerType_node = element.getElementsByTagName("routerType");
						String routerType = null;
						if (routerType_node.getLength()>0){
							Element routerType_e = (Element)routerType_node.item(0);
							routerType = getCharacterDataFromElement(routerType_e);
						}
						//Configuration Mode
						NodeList rotuerModel_node = element.getElementsByTagName("rotuerModel");
						String rotuerModel = null;
						if (rotuerModel_node.getLength()>0){
							Element rotuerModel_e = (Element)rotuerModel_node.item(0);
							rotuerModel = getCharacterDataFromElement(rotuerModel_e);
						}
						...
						node.setRotuerModel(rotuerModel);						
